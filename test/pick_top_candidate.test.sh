...
SCRIPT="${ROOT}/bin/pick_top_candidate.sh"
...
bash "${ROOT}/bin/active_smiths.sh" add smith-APP-5601 anderson-APP-5601 impl APP-5601
got=$(echo "$candidates" | bash "$SCRIPT")
assert_eq "APP-5612" "$got" "case2-skip-active"

# Case 3: all candidates already active → exit 1 (no candidate, signal to caller)
bash "${ROOT}/bin/active_smiths.sh" add smith-APP-5612 anderson-APP-5612 impl APP-5612
...
bash "${ROOT}/bin/active_smiths.sh" remove smith-APP-5601
bash "${ROOT}/bin/active_smiths.sh" remove smith-APP-5612
